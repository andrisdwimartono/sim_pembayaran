...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `summary_fx`  AS  select `c`.`GroupNasabah` AS `customer`,sum(`t`.`EkuivalentUSD`) AS `quantity`,count(`t`.`EkuivalentUSD`) AS `count`,year(`t`.`DealDate`) AS `year` from (`transaksi_fx` `t` join `customer` `c` on((`t`.`CusCod` = `c`.`CusCod`))) where (`t`.`Status` in ('INPUT','SUKSES KREDIT STO','SUKSES SISTEM LAIN','REJECT EC KREDIT')) group by `c`.`GroupNasabah`,year(`t`.`DealDate`) ;
--
