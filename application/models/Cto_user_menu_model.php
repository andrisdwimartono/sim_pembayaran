<?php
class Cto_user_menu_model extends MY_Model
{
	public function __construct() {
		parent::__construct();
	}
	
    protected $_table_name = 'cto_user_menu';
	protected $_cto_columns = array('controller', 'method', 'path', 'name', 'fk_menupackage_id', 'menupackage_name', 'sequence', 'is_shown', 'is_active');
	
	public function updateum($userid, $data){
		//check if user menu exist, then update else insert
		//first, update all to is_access to -1
		//then update again as same as the data
	...
	public function get_user_menus($username, $controller = null, $method = null){
        $query = $this->db->query("SELECT menus.html FROM (
									...
									SELECT 
										...
										CASE WHEN (cm.controller = '".$controller."' OR mpother.fk_menupackage_id = cmp.id) THEN CONCAT('<li class=\"treeview active\">
											  <a href=\"#\">
