...
class Cto_notification extends MY_Controller {
	...
	public function __construct() {
		...
		$this->load->model('cto_notification_model');
	}
	
	public function getAllData(){
		$position = $_SESSION['position'];
		$notif = $this->cto_notification_model->getAllData($position);
		
		echo json_encode($notif);
	}
	
	public function changeToWatched(){
		$data['is_watched'] = 1;
		$data['watched_by'] = $_SESSION['id'];
		
		try{
			if($this->cto_notification_model->update($this->input->post('id'), $data)){
				$pesan['messages'] = 'Data is saved';
				$pesan['url_redirect'] = $this->input->post("url_redirect");
				$pesan['status'] = true;
			}else{
